"""Native TimeNet connector for Penmanshiel Wind Farm SCADA & Status data."""
...
class PenmanshielConnector(BaseConnector[Path]):
    """TimeNet BaseConnector implementation for Penmanshiel wind farm SCADA & Status logs."""
    ...
    def download(self, cache_dir: Path) -> List[Path]:
        ...
        window_parquet_path = cache_dir / "penmanshiel_windows.parquet"

        if window_parquet_path.exists():
            return [window_parquet_path]

        # Generate or load windows across all 14 Penmanshiel turbines
        windows = generate_synthetic_scada_windows(
            farm="penmanshiel",
            turbines=PENMANSHIEL_TURBINES,
            windows_per_turbine=30,
            seed=42,
        )
    ...
    def convert(self, raw_refs: List[Path]) -> TimeFDataset:
        ...
        data_source = DataSource(
            ...
            name="Penmanshiel MM82 SCADA System",
        ...
        for _, row in df.iterrows():
            ...
            # Add annotations
            farm_anno = Annotation(key="wind_farm", value="Penmanshiel", id=f"farm-{rec_id}")
