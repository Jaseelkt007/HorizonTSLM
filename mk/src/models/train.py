...
def compute_class_weights(loader, num_classes: int, device: torch.device) -> torch.Tensor:
    """Compute balanced inverse-frequency class weights from training set."""
    labels = []
    for batch in loader:
    ...
    weights = np.clip(weights, 0.2, 5.0)  # Stabilize gradient scale
...
def main():
    ...
    if args.model_type == "softprompt":
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            val_loss, val_cls, val_lm, val_acc, val_f1 = evaluate_model_softprompt(
            ...
            print(
                ...
                f"Val Loss: {val_loss:.4f} (Cls: {val_cls:.3f}, LM: {val_lm:.3f}) F1: {val_f1:.4f}{star}",
                flush=True,
            )
        ...
        test_loss, test_cls, test_lm, test_acc, test_f1 = evaluate_model_softprompt(
