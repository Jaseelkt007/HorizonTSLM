...
set -u
export PATH="$HOME/.local/bin:$PATH" DATA_DIR="${DATA_DIR:-$HOME/data}" PYTHONUNBUFFERED=1
...
for job in "$@"; do
  ...
  uv run python -m turbine_tslm.training.train $cfg $extra > "outputs/$stem.log" 2>&1
