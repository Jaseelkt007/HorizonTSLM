...
# Compatibility for the initial XGBoost baseline. New producers should use the
# canonical ``label`` and ``score`` fields documented by this module.
def recall_at_false_alarm_rate(
    y_true: np.ndarray, scores: np.ndarray, far: float = 0.10
) -> float:
    """Recall at FAR used during XGBoost validation."""
    return recall_at_far(np.asarray(y_true, bool), scores, far)[0]


def score_predictions(labels: pd.DataFrame, predictions: pd.DataFrame) -> dict[str, Any]:
    """Score legacy ``pred_label``/``score_positive`` prediction frames."""
