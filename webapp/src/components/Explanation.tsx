...
/** The explanation with every numeric claim marked verified or wrong against the window. */
export default function Explanation({ body, claims, className }: { body: string; claims: Claim[]; className?: string }) {
  ...
  for (const c of claims) {
    ...
    parts.push(
      <mark key={c.start} className={`mark ${c.ok ? "" : "bad"}`} title={c.ok ? "verified against the window" : "does not match the window"}>
