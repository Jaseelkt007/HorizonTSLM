...
export default function HistoryStrip({ list }: { list: WindowSummary[] }) {
  ...
  const n = list.length;
  const W = 300, H = 40, top = 4, bh = 24, dotY = 36;
  const slot = W / n;
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} role="img" aria-label={`${n} sampled windows in time order`}>
      ...
      {list.map((w, i) => {
        ...
        const title = `${w.anchor} · asked ${w.horizon_h} h ahead · P(fault stop) ${fmt(w.score, 2)} · ${w.gold === "none" ? "no fault stop followed" : `${cls(w.gold)} stop followed`}`;
        return (
          <g key={w.id}>
            <rect className={styles.bar} x={x.toFixed(1)} y={top + bh - h} width={bw.toFixed(1)} height={h} rx={1} fill={w.score >= 0.5 ? "var(--risk)" : "var(--risk-track)"}
              onClick={(e) => { e.stopPropagation(); router.push(`/window/${w.id}/`); }}>
              <title>{title}</title>
            </rect>
